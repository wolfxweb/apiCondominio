...
$stmt->bindParam(13, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  e.plan_nome, t.* FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  e.plan_nome, t.* FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  WHERE LOWER(t.cond_nome) LIKE ? OR LOWER(t.cond_rua) LIKE ?  OR LOWER(t.cond_barirro) LIKE ?  OR LOWER(t.cond_cidade) LIKE ?  OR LOWER(t.cond_estado) LIKE ?  OR LOWER(t.cond_cep) LIKE ?  OR LOWER(t.cond_numero) LIKE ?  OR LOWER(t.cond_status) LIKE ?  OR LOWER(t.plan_id) LIKE ?  OR LOWER(t.cond_slug) LIKE ?  OR LOWER(t.cond_obs) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(13, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  e.plan_nome, t.* FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  e.plan_nome, t.* FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  e.plan_nome, t.* FROM ". $this->table_name ." t  join planos e on t.plan_id = e.id  WHERE t.plan_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
