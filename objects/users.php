...
public $cond_nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE LOWER(t.user_name) LIKE ? OR LOWER(t.user_email) LIKE ?  OR LOWER(t.user_password) LIKE ?  OR LOWER(t.user_cpf) LIKE ?  OR LOWER(t.user_rg) LIKE ?  OR LOWER(t.user_foto) LIKE ?  OR LOWER(t.nive_id) LIKE ?  OR LOWER(t.user_token) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.remember_token) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ? ";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(12, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE LOWER(t.user_name) LIKE ? OR LOWER(t.user_email) LIKE ?  OR LOWER(t.user_password) LIKE ?  OR LOWER(t.user_cpf) LIKE ?  OR LOWER(t.user_rg) LIKE ?  OR LOWER(t.user_foto) LIKE ?  OR LOWER(t.nive_id) LIKE ?  OR LOWER(t.user_token) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.remember_token) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(12, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
...
$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE t.user_email = ? AND t.user_password=? LIMIT 0,1";
$stmt = $this->conn->prepare($query);
...
$this->id=null;
	...
	function readOne(){
		$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  sss.cond_nome, t.* FROM ". $this->table_name ." t  join condominios sss on t.cond_id = sss.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
