...
public $nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  WHERE LOWER(t.nome) LIKE ?";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(1, $searchKey);
	...
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  t.* FROM ". $this->table_name ." t  WHERE LOWER(t.nome) LIKE ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(1, $searchKey);
	...
	function readOne(){
		$query = "SELECT  t.* FROM ". $this->table_name ." t  WHERE t.loj_id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(":nome", $this->nome);
		...
		if($stmt->execute()){
			...
			if($lastInsertedId==0 && $this->loj_id!=null){
				$this->readOne();
				if($this->loj_id!=null){
					$lastInsertedId=$this->loj_id;
					}
			}
		}
	...
	function update(){
		$query ="UPDATE ".$this->table_name." SET nome=:nome WHERE loj_id = :loj_id";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(":loj_id", $this->loj_id);
	...
	function update_patch($jsonObj) {
			...
			$query = $query . " " . $setValue . " WHERE loj_id = :loj_id"; 
			$stmt = $this->conn->prepare($query);
			foreach($jsonObj as $key => $value) 
			{
			    $columnName=htmlspecialchars(strip_tags($key));
	...
	function delete(){
		$query = "DELETE FROM " . $this->table_name . " WHERE loj_id = ? ";
		$stmt = $this->conn->prepare($query);
		$this->loj_id=htmlspecialchars(strip_tags($this->loj_id));
