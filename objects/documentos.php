...
public $cond_nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE LOWER(t.docs_id) LIKE ? OR LOWER(t.docs_titulos) LIKE ?  OR LOWER(t.docs_url) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ? ";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(6, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  fff.cond_nome, t.* FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  fff.cond_nome, t.* FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE LOWER(t.docs_id) LIKE ? OR LOWER(t.docs_titulos) LIKE ?  OR LOWER(t.docs_url) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(6, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  fff.cond_nome, t.* FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  fff.cond_nome, t.* FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  fff.cond_nome, t.* FROM ". $this->table_name ." t  join condominios fff on t.cond_id = fff.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
