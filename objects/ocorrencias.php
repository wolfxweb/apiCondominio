...
$stmt->bindParam(9, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE LOWER(t.ocor_id) LIKE ? OR LOWER(t.ocor_titulos) LIKE ?  OR LOWER(t.ocor_status) LIKE ?  OR LOWER(t.ocor_data_cadastro) LIKE ?  OR LOWER(t.ocor_fotos) LIKE ?  OR LOWER(t.unid_id) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(9, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE t.unid_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  xxx.unid_name, yyy.cond_nome, t.* FROM ". $this->table_name ." t  join unidades xxx on t.unid_id = xxx.id  join condominios yyy on t.cond_id = yyy.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
