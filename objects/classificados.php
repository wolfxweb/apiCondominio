...
public $cond_nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE LOWER(t.class_titulo) LIKE ? OR LOWER(t.class_fotos) LIKE ?  OR LOWER(t.class_descricao) LIKE ?  OR LOWER(t.class_preco) LIKE ?  OR LOWER(t.class_data) LIKE ?  OR LOWER(t.user_id) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ? ";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(9, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE LOWER(t.class_titulo) LIKE ? OR LOWER(t.class_fotos) LIKE ?  OR LOWER(t.class_descricao) LIKE ?  OR LOWER(t.class_preco) LIKE ?  OR LOWER(t.class_data) LIKE ?  OR LOWER(t.user_id) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(9, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE t.user_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  dddd.user_name, uu.cond_nome, t.* FROM ". $this->table_name ." t  join users dddd on t.user_id = dddd.id  join condominios uu on t.cond_id = uu.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
