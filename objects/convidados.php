...
public $cond_nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE LOWER(t.conv_nome) LIKE ? OR LOWER(t.conv_cpf) LIKE ?  OR LOWER(t.conv_rg) LIKE ?  OR LOWER(t.conv_telefone) LIKE ?  OR LOWER(t.conv_status) LIKE ?  OR LOWER(t.conv_data) LIKE ?  OR LOWER(t.conv_obs) LIKE ?  OR LOWER(t.unid_id) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ? ";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(11, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE LOWER(t.conv_nome) LIKE ? OR LOWER(t.conv_cpf) LIKE ?  OR LOWER(t.conv_rg) LIKE ?  OR LOWER(t.conv_telefone) LIKE ?  OR LOWER(t.conv_status) LIKE ?  OR LOWER(t.conv_data) LIKE ?  OR LOWER(t.conv_obs) LIKE ?  OR LOWER(t.unid_id) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(11, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE t.unid_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  fff.unid_name, gg.cond_nome, t.* FROM ". $this->table_name ." t  join unidades fff on t.unid_id = fff.id  join condominios gg on t.cond_id = gg.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
