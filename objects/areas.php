...
public $cond_nome;
	...
	function search_count($searchKey) {
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE LOWER(t.area_id) LIKE ? OR LOWER(t.area_titulo) LIKE ?  OR LOWER(t.area_status) LIKE ?  OR LOWER(t.area_fotos) LIKE ?  OR LOWER(t.area_days) LIKE ?  OR LOWER(t.area_time_start) LIKE ?  OR LOWER(t.area_time_end) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ? ";
		$stmt = $this->conn->prepare($query);
...
$stmt->bindParam(10, $searchKey);
	...
	function search_record_count($columnArray,$orAnd){
		...
		$query = "SELECT count(1) as total FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE ".$where."";
	...
	function read(){
		...
		$query = "SELECT  aaa.cond_nome, t.* FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		return $stmt;
	}
	function search($searchKey){
		...
		$offset = ($this->pageNo-1) * $this->no_of_records_per_page; 
		$query = "SELECT  aaa.cond_nome, t.* FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE LOWER(t.area_id) LIKE ? OR LOWER(t.area_titulo) LIKE ?  OR LOWER(t.area_status) LIKE ?  OR LOWER(t.area_fotos) LIKE ?  OR LOWER(t.area_days) LIKE ?  OR LOWER(t.area_time_start) LIKE ?  OR LOWER(t.area_time_end) LIKE ?  OR LOWER(t.cond_id) LIKE ?  OR LOWER(t.created_at) LIKE ?  OR LOWER(t.updated_at) LIKE ?  LIMIT ".$offset." , ". $this->no_of_records_per_page."";
		$stmt = $this->conn->prepare($query);
		$searchKey="%".strtolower($searchKey)."%";
...
$stmt->bindParam(10, $searchKey);
	...
	function searchByColumn($columnArray,$orAnd){
		...
		$query = "SELECT  aaa.cond_nome, t.* FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE ".$where." LIMIT ".$offset." , ". $this->no_of_records_per_page."";
	...
	function readOne(){
		$query = "SELECT  aaa.cond_nome, t.* FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE t.id = ? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
...
$this->pageNo =$_GET["pageNo"]; } 
$offset = ($this->pageNo - 1) * $this->no_of_records_per_page;
$query = "SELECT  aaa.cond_nome, t.* FROM ". $this->table_name ." t  join condominios aaa on t.cond_id = aaa.id  WHERE t.cond_id = ? LIMIT ".$offset." , ". $this->no_of_records_per_page."";
